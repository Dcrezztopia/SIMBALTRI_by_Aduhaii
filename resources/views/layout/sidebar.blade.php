<aside id="sidebar" class="sidebar">
    ...
    <ul class="sidebar-nav" id="sidebar-nav">
        <a href="{{ route('home') }}">
            <img class="my-3 ml-2" src="{{ asset('assets/img/simbaltri2.png') }}" alt=""
                style="width: 150px; height: 300;">
        </a>
        {{--<!-- <img class="my-3 ml-2" src="{{ asset('assets/img/simbaltri2.png') }}" alt="" style="width: 150px;
        height: 300;"> -->--}}
        ...
        <li class="nav-item">
            <a class="nav-link @if($activeSidebarItem[0] == 'home') active @else collapsed @endif"
                href="{{ route('warga.home') }}">
                <i class="bi bi-house"></i>
                <span>Home</span>
            </a>
        </li>
        ...
        <li class="nav-item">
            ...
            <a class="nav-link collapsed @if($activeSidebarItem[0] == $key) active @endif"
                href="{{ route($item['route']) }}">
                <i class="{{ $item['icon'] }}"></i><span>{{ $item['label'] }}
            </a>
            @else
            <a class="nav-link collapsed @if($activeSidebarItem[0] == $key) active @endif"
                data-bs-target="#{{ $key }}-nav" data-bs-toggle="collapse" style="cursor: pointer">
                <i class="{{ $item['icon'] }}"></i><span>{{ $item['label'] }}</span><i
            ...
            <ul id="{{ $key }}-nav" class="nav-content collapse" data-bs-parent="#sidebar-nav">
                ...
                <li>
                    <a class="nav-link @if($activeSidebarItem[0] == $key && $activeSidebarItem[1] == $childKey) active @endif"
...
#something .sidebar-nav .nav-link.active::after {
    ...
    display: block;
    width: 100%;
    /* Ubah ini sesuai dengan panjang garis yang diinginkan */
